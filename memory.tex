...
智能系统中记忆的设计似乎是一个比较 open-ended question。 暂时我列出一些关於记忆的点子，但仍未综合成一个系统的理论....

\setcounter{section}{-1}
\section{Turing machine 的启示}
...
\begin{equation}
\includegraphics[scale=0.7]{Turing-machine.png}
\end{equation}
...
在 \cite{Yan2017} 中我们提出了智能系统的 minimal architecture，但是它没有对\textbf{历史}的记忆。 换句话说，只能留意\textbf{当下}发生的事件，但不能记住一段\textbf{故事}。
...
换句话说，最简单的 \emp{reactive system} 其实是一个只有\textbf{瞬时记忆}和\textbf{永恒记忆}的系统。 例如无人驾驶车，它的瞬时记忆就是路面状态，而永恒记忆是「转弯、打灯、泊车」等动作。 它根据路面状况（例如有没有撞车等奖励／惩罚），学习永恒记忆中的 responses。 

我们的目标是要设计有 \emp{limited memory} 的系统，令系统能够根据 episodic memory 来学习。
...
%\end{equation}
Z-transform 的\textbf{连续时间}版本是 Laplace transform （不知有没有用？）
...
\begin{itemize}
\item able to recognize （\textbf{辨识}） patterns within time and across time
\item cue-based retrieval （或者说，基於 \textbf{attention} 的记忆读取）
\item mental state \textbf{分拆}为一束命题的集合
\item \textbf{遗忘}的机制，with graceful degradation
\item \textbf{hierarchical} 组织（基於时间或其他 attributes）
\item 
\end{itemize}
...
\begin{equation}
\includegraphics[scale=0.75]{memory-module.png}
\end{equation}

\begin{itemize}
\item 记忆应该如何组织？
\item retrieval 根据的是什么？ \\
应该是 relevance。 注意： rules 在 $\KB$ 里，facts 在 memory 里。 这个 config 要保持吗？
\item 
\end{itemize}
