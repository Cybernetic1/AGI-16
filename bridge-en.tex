...
Consider, for example, to recognize \textit{``white cat chasing black cat''}.  The \textit{``cat''} object has to be recognied twice;  Obviously we should not waste 2 neural networks to do this.  If we connect a neural network \textbf{head to tail}, we get a minimalist cognitive architecture with a \textbf{recurrent} loop:
\begin{equation}
...
\end{equation}
For example, the 3 {\color{red}red} dots represent $\{ A \}$, $\{ B \}$, and $\{ A, B \}$.  The diagonal elements are not needed, because $(A, A)$ etc. are \textit{redundant} repetitions.  In other words, all the propositional combinations are:
\begin{equation}
\{ \mbox{ lower-triangular elements } \} \; \setminus \mbox{ diagonal } \cup \; \emptyset
\end{equation}
...
In high dimensions, this space-saving scheme is extremely efficient:  After symmetrization, a ``corner'' of the hypercube has a volume that is only $1/n!$ of the original cube.  Even as the \textbf{curse of dimensionality} grows exponentially, $n!$ is well sufficient to offset it.
...
\subsubsection{Trading space with time}
...
\subsubsection{Application to belief revision}
