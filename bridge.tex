...
\emp{动态规划} (dynamic programming) 的中心思想是 Bellman equation； 我们根据 Bellman update 寻找状态空间中的\emp{最优路径}。
...
另一方面，逻辑是几百年来发展起来的关於人类思考的规律； 逻辑的描述是正确的； 逻辑和神经之间必然有一个 correspondence，因为它们都在做同样的事： 智能。 
...
\end{equation}
假设红色的神经元专门负责辨识「对角线」这一特徵。 它的方程式是 $\vect{y} = \sigmoid (W \vect{x})$。  矩阵 $W$ 的作用是 affine「旋转」特徵空间，令我们想要的特徵集中在某一方向。 然后再用 $\sigmoid$ 「\textbf{扯开}」想要的特徵和不想要的特徵\footnote{所谓「扯」(stretch) 的意思是说，将本来邻近的两点的距离非线性地拉远。 看看以下各种常见的激活函数，它们全都是相对於 identity $y = x$ 的非线性 distortion：
\begin{equation}
...
换句话说： \underline{每个神经元的输出其实代表某个 feature 的存在与否}。\\
...
\end{eqnarray}
但要注意的是这对应未必是一对一的，可能是一个 constant 对应几个 neurons 的\textbf{线性组合}。
