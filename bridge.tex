...
\item 多个命题的集合（= 记忆的结构）
\end{itemize}
以下分述之。 谓词逻辑的命题结构比较特别，所以我初时专注於分析它，但后来发现它的影响是次要的； 最重要的是记忆的结构 (\S\ref{sec:memory})。
...
逻辑命题的\textbf{内部}结构，传统上有两种方法表达： 一是 predicate logic，二是 relation algebra。 无论是哪种方式，它的本质都是想处理 \emp{variable substitution}。 有趣的是，「代数」的本质其实也是「代入」，但代数中的代入法则是 implicit 的，所以用代数来 explicitly 表达代入的概念反而很难。
...
(First-order) predicate logic 的代数化，可以由 Alfred Tarski 的 \emp{cylindrical algebra} 给出。 Higher-order logic (HOL) 的代数化可以用 \emp{topoi theory} 给出。  HOL 等效於 untyped $\lambda$-calculus，而 typed $\lambda$-calculus (= type theory) 较为弱一点。  HOL 这个家族的特点，是 ``substitution of equal by equals''，这会令逻辑式子的长度增加。 但 predicate logic 的做法是用\textbf{实物} (objects) 来做替换，它不会增加式子的长度； 但将它代数化的结果是引入了 cylindrification、diagonal set 等概念，这可以在 fig. (\ref{fig:diagonals}) 中看到一些影子。 

First-order logic (FOL) 处理 higher-order 关系时不方便，但我选择了 FOL 是因为那 linkage 结构用神经网络似乎较易处理。 至於 higher-order relations 则可以用「时间」来搭救（即\textbf{分开数个步骤}进行）。 
